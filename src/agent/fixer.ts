 ...
 */
export async function runFixMode(issue: string, context: any, rl?: readline.Interface, onToken?: (token: string) => void, onTool?: () => void) {
...
Always double check filenames, path details, and patch content for accuracy.`;
  ...
  return runDynamicAgent(issue, systemInstructions, context, rl, onToken, onTool);
}
