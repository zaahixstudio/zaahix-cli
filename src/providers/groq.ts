  ...
  private async askWithRetry(
    ...
    try {
      ...
      const model = mode === "chat"
        ? (process.env.GROQ_CHAT_MODEL || process.env.GROQ_MODEL || "llama-3.1-8b-instant")
        : (process.env.GROQ_CODE_MODEL || process.env.GROQ_MODEL || "llama-3.3-70b-versatile");
      ...
      if (err?.status === 429 || errStr.includes("429") || errStr.includes("Rate limit")) {
        if (retryCount >= 5) {
          return `❌ Groq Rate Limit Error: Exceeded maximum retries (5). Please wait a moment and try again.`;
        }
      ...
      return `❌ Groq Error: ${err?.message || err}`;
    }
  }
}
